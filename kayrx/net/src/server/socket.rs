...
use crate::server::codec::{AsyncRead, AsyncWrite};
...
#[cfg(all(unix))]
impl FromStream for crate::server::rt::net::UnixStream {
    fn from_stdstream(sock: StdStream) -> io::Result<Self> {
        match sock {
            StdStream::Tcp(_) => panic!("Should not happen, bug in server impl"),
            StdStream::Uds(stream) => crate::server::rt::net::UnixStream::from_std(stream),
        }
    }
}
